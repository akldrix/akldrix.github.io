...
function deleteFromCart(item, price) {
    ...
    showNotification(`${item} удален из корзины!`);
...
function showPage(page) {
    ...
    if (page === 'menu') {
        ...
        updateCart(); // Обновить корзину при открытии страницы
...
function placeOrder() {
    ...
    if (cart.length === 0) {
        ...
        showNotification("Ваша корзина пуста. Пожалуйста, добавьте товары в корзину!");
...
function clearCart() {
    ...
    showNotification('Корзина очищена!'); // Уведомляем пользователя
